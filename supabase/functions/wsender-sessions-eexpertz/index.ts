...
serve(async (req) => {
  ...
  try {
    ...
    if (override && !override.includes("-eexpertz")) {
      override = override.replace("webhook-wsender", "webhook-wsender-eexpertz");
    }
    const webhookUrl = override || `${supabaseUrl}/functions/v1/webhook-wsender-eexpertz`;
