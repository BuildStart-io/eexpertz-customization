...
const SOURCE_SYSTEM = "eexpertz";
const TARGET_SCHEMA = "eexpertz_customization";
...
serve(async (req) => {
  ...
  try {
    ...
    // We use the service role key to bypass RLS and explicitly target the isolated schema
    const supabase = createClient(supabaseUrl, supabaseServiceKey, { db: { schema: TARGET_SCHEMA } });

    console.log(`Starting CRM usage sync for ${SOURCE_SYSTEM}...`);
